...
So here's an example of a finite automaton. We start in the start state, and we read strings of symbols over the alphabet **1, 2, 3**. And we accept if and only if the first letter is the same as the last letter. So the states labeled **1, 2, and 3**.
**525.21**: represent that this was the first symbol you saw. 

**528.68**: And then if you ever return to that, 

**531.04**: that what you've read so far is accepted. 

**533.42**: Now of course, you might leave that state later on 

**535.9**: if it looks like 1, 2, 3, 1, 2 and end up not accepting, 

**540.38**: but that's the way it works. 

**541.97**: And the convention is that a **double circle** indicates a final state. 

**546.76**: You can also use **automata** to compute finite range functions. 

**551.12**: So functions that take their values in a finite set. 

**554.15**: And this is a classic example from a first automata theory course in the **undergraduate computer science curriculum**, 

**562.0**: is you have an automaton which computes **n modulo three** 

**566.35**: when the input is n in **base 2**. 

**568.49**: And you have a state for each of the **residue classes**, 

**571.88**: and, for example, if you're in the residue class 1, then when 

**578.29**: you read a 0, that's like multiplying the number you've seen so far by 2, 

**582.88**: so you should go to the residue class 2. 

**585.253**: OK. 

**589.48**: So, what's **Walnut**? 

**592.16**: Walnut is what I'm going to be talking about a lot of the time. 

**595.04**: It's an **open-source theorem prover** that's based not just on **Presburger arithmetic**, 

**601.19**: but on a certain extension of **Presburger arithmetic**. 

**604.4**: So with Presburger arithmetic, you can do some things, 

**608.39**: but I would say not that much of genuine mathematical interest on its own. 

**614.57**: That is, the kinds of things that you can state are not typically the kinds of theorems you would publish in a journal. 

**621.29**: It is useful, nevertheless, for people doing things like analysis of finite-state systems and **program analysis** 

**630.1**: and so forth, but it doesn't really result in publishable pure mathematics. 

**636.79**: So Walnut can prove these claims. 

**640.64**: And it's not a general purpose proof assistant or anything like that. 

**645.63**: It's been used in lots of papers and books. 

**648.42**: There's a list on my **web page** that you can get to of over **100 papers and books**. 

**654.36**: And the kinds of things it can do is verify existing theorems. 

**659.22**: So sometimes there'll be a long proof, tedious proof by induction with many cases, and you can just 

**665.51**: replace it all just by stating the theorem. 

**670.19**: We found some false results in the literature, 

**672.93**: and I wouldn't claim they are important errors. 

**676.62**: They're easily correctable errors, 

**678.27**: but it's fun that it was able to find them. 

**681.98**: And on several occasions, we've been able to resolve some previously unproved conjectures and proving 

**688.67**: lots of new results. 

**690.83**: And in some cases, the proofs generated by **Walnut** are still the only known proofs for the claims. 

**698.19**: So just to see what it looks like when you actually use it, 

**703.71**: the black line at the top is this the **Chicken McNuggets theorem**, 

**707.33**: and the part in red is how you would type it in in Walnut. 

**712.68**: And I just want to point out a couple of things. 

**715.28**: One is that you have some **eval** that says evaluate what follows as a statement with no free variables. 

**725.78**: And it's called— and call it something— in this case, **chicken**. 

**729.12**: And then "for all," our philosophy was we wanted everything in just— oops. 

**735.02**: We wanted everything in just plain **ASCII** with no special keyboards or anything so that anybody could use it on anything. 

**743.39**: And so instead of having a nice "for all" system symbol like you do in **Lean**, 

**749.46**: we just have a capital A, which is just "for all" system symbol upside down. 

**754.61**: And the same thing for "there exists," which is just a capital E. 

**760.53**: So you can see that it's basically the same thing. 

**761.53**: We use the **ampersand** for the "and," we use the **twiddle** for the "not," and it says true. 

**768.37**: So indeed, the **Chicken McNuggets theorem** is true. 

**773.92**: OK, so what system is it based on? 

**777.25**: It's based on this small extension of **Presburger arithmetic**. 

**780.7**: The small extension of **Presburger arithmetic** just adds the ability to compute a certain function **Vk** of n. 

**790.41**: **Vk** of n is the largest power of k dividing n. 

**795.13**: I don't mean the exponent of the largest power, 

**798.07**: but the power itself.
So for example, **V2 of 24** is **8** because 8 is the largest power of 2 dividing 24. 
...
So now we can express that it's unbordered by saying, well,
1347.72 - 4.74: No matter what **j** you look at, from **1 to n** over **2**, 

1352.46 - 3.57: then it's always not bordered. 

1356.03 - 2.24: So there's another **logical formula**. 

1358.27 - 4.65: And then finally, we can say **t** has an **unbordered factor** 

1362.92 - 2.13: of length **n**; we just say, well, there 

1365.05 - 2.79: exists some **i**, some starting position, 

1367.84 - 3.73: such that the factor of that length is unbordered. 

1371.57 - 2.49: OK, so we translate this all to **Walnut**. 

1374.06 - 3.41: Here, the **black line** is the first order **logical statement** 

1377.47 - 3.72: in normal mathematical notation, and the **red line** below it 

1381.19 - 2.17: is how you would type it into **Walnut**. 

1383.36 - 2.66: And I think it's pretty clear here. 

1386.02 - 2.19: I've used **def** because I'm defining 

1388.21 - 6.51: an **automaton** for later use that implements 

1394.72 - 4.65: this first-order predicate in the way that I said. 

1399.37 - 4.75: So, as you can see, it looks pretty much the same. 

1404.12 - 3.35: So it's not like it's a lot of work translating these things 

1407.47 - 2.89: into **Walnut's language**. 

1410.36 - 2.25: And then finally, at the very end, we get **Currie**. 

1412.61 - 2.48: **Currie** is the property that-- 

1415.09 - 3.21: **Currie of n** tells you whether there's an **unbordered factor** 

1418.3 - 1.0: of length **n**. 

1419.3 - 2.64: And now we can prove the **Currie-Saari theorem**. 

1421.94 - 3.96: So it says if **n** is not congruent to **1 mod 6**-- 

1425.9 - 2.69: and here, I've implemented **mod** just 

1428.59 - 3.88: by saying **n minus 6 times the floor of n over 6**. 

1432.47 - 1.65: So that's **n mod 6**. 

1434.12 - 4.04: If **1** doesn't equal that, then we should get-- 

1438.16 - 3.31: there's an **unbordered factor** of length **n**, and **Walnut** says true, 

1441.47 - 0.96: and so it's proved. 

1442.43 - 1.82: Yeah? 

1444.25 - 3.39: AUDIENCE: Maybe this is a part, but what **base** 

1447.64 - 1.042: is the factor written in? 

1448.682 - 0.916: **JEFFREY SHALLIT**: Yeah. 

1449.598 - 2.582: So the default, if I don't say anything, it's always **base 2**. 

1452.18 - 2.28: But you can say any **base**. 

1454.46 - 1.97: AUDIENCE: So this is true for any base that-- 

1456.43 - 3.0: the fact of maybe we can-- 

1459.43 - 2.46: **JEFFREY SHALLIT**: Numbers are independent of the base they 

1461.89 - 5.07: are written in, in general, but in **Walnut**, you have 

1466.96 - 2.62: to declare some **base** to use it. 

1469.58 - 3.678: But the **theorem** doesn't depend on the **base**, if I-- 

1473.258 - 2.042: AUDIENCE: I just meant that it's possible that it 

1475.3 - 2.95: is **unbordered** in one **base**, but might have a border-- 

1478.25 - 1.91: **JEFFREY SHALLIT**: No. 

1480.16 - 3.52: **n** in **base 3** is the same **n** as in **base 2**. 

1483.68 - 5.39: It doesn't-- I'm talking about **n**, not its representation. 

1489.07 - 3.55: AUDIENCE: Well, but a border is just the **digits** at the end, 

1492.62 - 2.36: so they might be the same in one base, but not-- 

1494.98 - 1.89: **JEFFREY SHALLIT**: The **border**-- 

1496.87 - 3.31: so I obviously didn't explain it so well. 

1500.18 - 2.21: The **string** is just a-- that we're 

1502.39 - 2.31: talking about is a specific **string**, 

1504.7 - 6.28: and the **border** is a **string**, not an **integer**, a block of letters. 

1510.98 - 2.4: So it's **0's and 1's**. 

1513.38 - 3.38: It has no **base** associated with the **0's** and the **1's**. 

1516.76 - 2.53: They are just **symbols**. 

1519.29 - 1.03: Does that answer it? 

1520.32 - 0.5: Yeah. 

1520.82 - 0.992: If not, come see me. 

1521.812 - 1.708: AUDIENCE: Or is it just equivalent to say 

1523.52 - 4.155: that the integer equivalent binary has this report problem? 

1527.675 - 1.125: **JEFFREY SHALLIT**: I'm sorry? 

1528.8 - 2.88: AUDIENCE: So **t** is a 

1531.68 - 5.173: sequence that's not related to any number space? 

1536.853 - 0.917: **JEFFREY SHALLIT**: Yeah. 

1537.77 - 4.99: I don't quite see what the issue is, but the **base** has nothing 

1542.76 - 0.5: to-- 

1543.26 - 5.58: I mean, neither-- the **0**, **1**, **0** at the beginning or end 

1548.84 - 2.35: or something is not a **number**. 

1551.19 - 4.01: It's just **0**, **1**, **0**, it has no **base** associated with it. 

1555.2 - 0.72: OK. 

1555.92 - 2.25: Anyway. 

1558.17 - 5.19: So, we can do a lot more than that because it turns out that 

1563.36 - 1.935: their theorem was not an "if and only if." 

1568.592 - 4.098: That there are **words** of length congruent to **1 mod 6**, like this one 

1572.69 - 2.64: right here, which is an **unordered factor of length 31**. 

1577.963 - 2.167: So, I mean, maybe you have to do a little work there 

1580.13 - 2.8: to convince yourself that it's unbordered, but it is. 

1582.93 - 2.94: So that appears somewhere in the **Thue-Morse sequence**. 

1585.87 - 2.09: And so we didn't get an "if and only if." 

1587.96 - 4.11: But getting the "if and only if" is exactly what **Walnut** is good 

1592.07 - 0.61: at. 

1592.68 - 5.49: And because **Currie** turns out to be just a **six-state automaton**. 

1598.17 - 1.71: And what does it represent? 

1599.88 - 1.13: What does it recognize? 

1601.01 - 2.94: It recognizes the **base 2 representation** 

1603.95 - 5.64: of all **n** for which **t** has a length **n unbordered factor**.
OK.
...
**SPEAKER:** So, the next talk is by **Emily Riehl**. 
