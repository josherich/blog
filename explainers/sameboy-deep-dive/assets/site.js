(function () {
  ...
  function setupControls() {
    ...
    if (expand) {
      expand.addEventListener("click", function () { setDetails(true); });
    }
    if (collapse) {
      collapse.addEventListener("click", function () { setDetails(false); });
    }
  }
    ...
    all("[data-flow-step]").forEach(function (step) {
      step.addEventListener("click", function () {
        ...
        all("[data-flow-target]").forEach(function (item) {
          ...
          if (item.getAttribute("data-flow-target") === key) {
            item.open = true;
