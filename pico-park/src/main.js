...
let settingsOpen=false,settingsWasPaused=false;
...
function stageButtons(){
 const board=$('stages');
 if(board){board.replaceChildren();stages.forEach((s,i)=>{const b=document.createElement('button');b.className='stage'+(i===index&&!custom?' active':'')+(saved[s.id]?' cleared':'');b.textContent=String(s.id).padStart(2,'0');b.title=s.name+(saved[s.id]?' · Best '+timeString(saved[s.id]):'');b.setAttribute('aria-label',`Stage ${s.id}: ${s.name}`);b.setAttribute('aria-pressed',String(i===index&&!custom));b.onclick=()=>{custom=null;start(i);};board.append(b);});}
 const progress=$('progress');if(progress)progress.innerHTML=String(stages.filter(s=>saved[s.id]).length).padStart(2,'0')+` <span>/ ${stages.length}</span>`;
}
function start(i=index){index=i;selected=0;world=new World(custom||stages[index],count);paused=false;overlayMode='';keys.clear();pressed.clear();$('overlay').classList.add('hidden');stageButtons();canvas.focus({preventScroll:true});}
...
function togglePause(){if(world.won||world.failed)return;if(paused){paused=false;overlayMode='';$('overlay').classList.add('hidden');canvas.focus();}else{overlay('pause','Paused.','Your crew will be right here.','Keep playing →','TAKE A BREATHER');}}
function openSettings(){if(settingsOpen)return;settingsOpen=true;settingsWasPaused=paused;$('settings').classList.remove('hidden');$('gear').setAttribute('aria-expanded','true');if(!paused){paused=true;keys.clear();pressed.clear();}$('settings-close').focus({preventScroll:true});}
function closeSettings(){if(!settingsOpen)return;settingsOpen=false;$('settings').classList.add('hidden');$('gear').setAttribute('aria-expanded','false');keys.clear();pressed.clear();if(!settingsWasPaused&&!overlayMode)paused=false;canvas.focus({preventScroll:true});}
...
window.addEventListener('keydown',e=>{
 if(settingsOpen){if(e.code==='Escape'&&!e.repeat)closeSettings();return;}
 if(['TEXTAREA','INPUT','SELECT'].includes(e.target.tagName))return;
 if(e.target.tagName==='BUTTON'&&['Space','Enter'].includes(e.code))return;
 if(e.code==='Tab'&&e.target!==canvas)return;
 if([...bindings.flat(),...(world.sharedControl?sharedBindings.flat():[]),'Tab','Space','Escape'].includes(e.code))e.preventDefault();
 if(e.code==='Tab'&&!paused&&!world.sharedControl){selected=(selected+(e.shiftKey?count-1:1))%count;keys.clear();pressed.clear();}
 else if(e.code==='KeyR'&&!e.repeat)start();
 else if(e.code==='Escape'&&!e.repeat)togglePause();
 else {keys.add(e.code);if(!e.repeat)pressed.add(e.code);}
});
...
$('resume').onclick=()=>{if(overlayMode==='pause')togglePause();else if(overlayMode==='won'&&!custom)start((index+1)%stages.length);else start();};
