...
function readInputs(){
 const inputs=Array.from({length:count},()=>({}));bindings.forEach((binding,i)=>{const dest=i===0?selected:i;if(dest>=count)return;const a=inputs[dest];['left','right','jump','use'].forEach((v,k)=>a[v]=a[v]||(keys.has(binding[k])||pressed.has(binding[k])));});
 const pads=navigator.getGamepads?.()||[];let slot=0;for(const pad of pads){if(!pad)continue;const dest=((count>4?4:0)+slot++)%count;const a=inputs[dest];a.left||=pad.axes[0]<-.25||pad.buttons[14]?.pressed;a.right||=pad.axes[0]>.25||pad.buttons[15]?.pressed;a.jump||=pad.buttons[0]?.pressed;a.use||=pad.buttons[1]?.pressed;}
 return inputs;
}
function update(dt){
 ...
 if(world.won){if(!custom){const prev=Number(saved[world.stage.id]);saved[world.stage.id]=prev?Math.min(prev,world.time):world.time;try{localStorage.setItem('pico-park-v1',JSON.stringify(saved));}catch{}stageButtons();}const last=index===stages.length-1;overlay('won',last&&!custom?'Home, together.':'Teamwork looks good.',`All ${count} cats made it home in ${timeString(world.time)}.${last&&!custom?' You finished the final Classic stage.':''}`,custom?'Play again ↻':last?'Back to stage 01 →':'Next adventure →','STAGE CLEAR');}
}
window.addEventListener('keydown',e=>{if(['TEXTAREA','INPUT','SELECT'].includes(e.target.tagName))return;if(e.target.tagName==='BUTTON'&&['Space','Enter'].includes(e.code))return;if(e.code==='Tab'&&e.target!==canvas)return;if([...bindings.flat(),'Tab','Space','Escape'].includes(e.code))e.preventDefault();if(e.code==='Tab'&&!paused){selected=(selected+(e.shiftKey?count-1:1))%count;keys.clear();pressed.clear();}else if(e.code==='KeyR'&&!e.repeat)start();else if(e.code==='Escape'&&!e.repeat)togglePause();else {keys.add(e.code);if(!e.repeat)pressed.add(e.code);}});
