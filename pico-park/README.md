...
Walk through friends horizontally, land on their heads vertically, and jump off them. Numbered gates need the indicated crew nearby, and stay open. Crates need the indicated number pushing. A collected key is shared. Every cat must use the door to finish; cats inside no longer block others.
...
| --- | --- | --- |
| 01 | Two numbered barriers; key left, exit right | Proportionally scaled proximity gates, permanently open once activated |
| 02 | Low floor, switch by exit, high key | Latched switch and rising key lift |
...
| 05 | Two numbered platforms and high exit | Group lifts and a moving platform |
...
| 09 | Individual clocks and timing objective | Stop each clock at 5 ± 0.5 seconds; use again to reset |
...
| 18 | Gap and fan beside high key | Horizontal ferry and upward wind volume |
| 19 | Colored falling-block compartments, target 10 | Each player rotates a piece to its outline; ten team rows |
| 20 | Two gates, light and “DON’T PUSH!” button | Key route, scaled gates, green/red movement rule; button triggers red |

The tetris, tower, timer and stop-light rules are interpretations of the visible cues. Future fidelity work can replace stage data or the corresponding mode without rewriting movement or UI. References were inspected September 9, 2026.
...
Every entity has `type`, `x`, `y`; optional `w`, `h` override default dimensions. `need` is a ten-player reference value: runtime requires `ceil(need × playerCount / 10)`, at least one. An `id` connects buttons to lifts.

| Type | Additional properties | Behavior |
| --- | --- | --- |
| `gate` | `need` | Opens when enough cats are within 75 px horizontally and overlap its vertical span |
| `crate` | `need` | Solid, gravity-driven, pushes when enough nearby cats walk into it |
| `lift` | `need`, `toY`, optional `id` | Counts cats above its deck, rises at 65 px/s and latches |
| `moving` | `axis: "x" / "y"`, `travel`, `speed` | Sinusoidal motion from origin; x travels right, y travels upward |
| `button` | `target: "entity-id"` | Standing on it permanently activates its target lift |
...
| `fan` | — | Updraft extends 180 px above its volume |
