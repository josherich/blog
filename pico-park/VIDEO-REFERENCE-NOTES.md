...
| [11](https://www.youtube.com/watch?v=9BjWF34lYgU&t=20s) | 00:20 / 00:35 / 00:50: a raised cannon on the right sends a ball toward a locked key box on the left while the cats position beneath it. The ball is relayed across character bodies and is lost on floor contact. | Removed the invented raised-basket pickup puzzle. Added the caged key, raised cannon, character-only ball bounce, floor-contact retry and airborne delivery objective. Projectile speed and bounce are tuned approximations. |
| [13](https://www.youtube.com/watch?v=YfApMosfAw8&t=8s) | 00:08 / 00:22 / 00:39: two cats are joined by an elastic wire above a large solid center shape; one hangs from the left edge to fetch the key, the segmented right hatch clears, and the route continues through the underpass into the narrow inner corridor. | Rebuilt the platforms as the observed center mass, left wall, underpass and inner corridor. Added a Stage 13 tether constraint and keyed segmented hatch. Elastic strength and scaled dimensions are tuned approximations. |
...
- Stages 3, 4, 6, 10 and 12 have linked playlist clips but were not given a complete gameplay audit in this revision. Their original screenshot-based limitations remain.
...
## Stages 14 and 17 correction — September 11

- [Stage 14](https://www.youtube.com/watch?v=8oaU6eOy2tI&t=38s): inspected 00:05 and 00:38. Rebuilt the sealed lower room, separate upper shelf and pipe, and exterior ghost. The user's clarified rule drives the implementation: any player looking toward the ghost stops it and cycles the four direction choices; all players must look away for movement to resume in the selected direction. Facing persists while idle. The ghost carries the key into the pipe, which releases it inside the room. The reference initially shows a separate key; this version starts it on the ghost as requested. Speed, arrow order/cadence and pipe delivery animation are adaptations.
- [Stage 17](https://www.youtube.com/watch?v=9eKGUVBOBKM&t=12s): inspected opening gameplay and 00:12 / 00:20. Rebuilt the three bottom stones, four descending left stair blocks, checkpoint ledge, rimmed middle platform, two upper-left shelves, pipe pedestal, center-right shelf, upper-right pair of blocks and final flag ledge. Replaced the three invented lifts with the two visible vertical decks; removed the unsupported spring. Coordinates are scaled to the canvas. Lift timing and travel are approximate; the visible enemy is not implemented.

Validation: regression tests cover one watcher among 2/10 players, all four stopped arrows, persistent idle facing, resumed movement, solid ghost boundaries, input-driven steering through the pipe and a complete stage 14 clear. Stage 17 tests verify landings on small stones/stairs, open floor gaps and checkpoint recovery. Both stages were visually checked in the browser. This is not a complete stage 17 multiplayer clear proof.

Stage 17 follow-up: the user clarified that the top-center gamepad requires matching inputs from both players to control a single character. Added a shared-control mode, five distinct input votes, controller highlighting, one-body checkpoint/key/exit handling, and matching-input regression tests. For crews larger than two, all selected controllers must agree. Up is mapped to jump and Down/Use to interaction in this platformer adaptation.
