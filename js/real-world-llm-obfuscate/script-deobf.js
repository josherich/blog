...
(() => {
  "use strict";
  const { Readability, isProbablyReaderable } = require('readability-js');
  let shadowRootId = "readergpt__shadowRoot";
  var parsedContent = new e.Readability(document.cloneNode(!0))
    .parse();
  function getCSSTag(e) {
  ...
  function showError(e) {
    var t, i, a;
    null === (t = getElement("readergpt__readableText")) || void 0 === t || t.classList.remove("d-none"), null === (i = getElement("readergpt__readableText")) || void 0 === i || i.classList.remove("alert-secondary"), null === (a = getElement("readergpt__readableText")) || void 0 === a || a.classList.add("alert-warning"), getElement("readergpt__readableText")
        .innerText = e
  }
  function hideLoading() {
    var e, t, i;
    null === (e = getElement("readergpt__gifContainer")) || void 0 === e || e.classList.add("d-none"), null === (t = getElement("readergpt__gifContainer")) || void 0 === t || t.classList.remove("loading"), null === (i = getElement("readergpt__loginContainer")) || void 0 === i || i.classList.remove("loading")
  }
  ...
  chrome.runtime.onMessage.addListener(((request, t, sendResponse) => {
        "getReadableContent" !== request.command || sendResponse(parsedContent)
    }));
  (function initialize() {
      var e, i;
      if (document.getElementById(shadowRootId)) return;
      let rootContainer = document.createElement("div");
      rootContainer.id = shadowRootId;
      rootContainer.style.cssText = "font-size: 16px;";
      rootContainer.attachShadow({
        mode: "open"
      });
      rootContainer.shadowRoot?.appendChild(getCSSTag("content.css"));
      rootContainer.shadowRoot.appendChild(getCSSTag("popup.css"));

      let containerEl = document.createElement("div");
      containerEl.id = "readergpt__container";
      containerEl.className = "readergpt__container readergpt-styles";

      let s = chrome.runtime.getURL("svgfiles/aigpt.svg");
      let l = chrome.runtime.getURL("settings.html");
      containerEl.innerHTML = `
<nav class="navbar navbar-light bg-light sticky-top pt-0 pb-0">
  <div id="readergpt__navContainer" class="container">
    <div class="navbar-brand"> <img src="${s}" style="width: 24px; height: 24px;"
        class="d-inline-block align-text-middle"> <span id="readergpt__navbarTitle" class="align-middle h6">Summary with
        ChatGPT</span> </div> <button id="readergpt__closeButton" type="button" class="btn btn-close d-flex"></button>
  </div>
</nav>
<div id="readergpt__summarySection">
  <div class="card d-none" id="readergpt__loginContainer">
    <div class="card-body">
      <p class="card-text">Please <a href="https://chat.openai.com" target="_blank" rel="noreferrer"
          id="readergpt__loginButton" class="card-link">Login</a> to OpenAI. Do refresh this page after login.</p>
      <p class="card-text mt-2"><small class="text-muted">ChatGPT requires logging in to OpenAI once in a while to
          use.</small> </p>
      <div class="alert alert-success" role="alert"> Now you can use OpenAI API Key to avoid repeat login and super fast
        response.<br /><a href="${l}" target="_blank" class="alert-link">Click here</a> for one time setup. </div>
      <p id="readergpt__braveWarning" class="card-text mt-2 alert alert-warning d-none"> To use it on Brave, please
        ensure that the <a href="https://i.imgur.com/9F6cTvD.png" target="_blank">fingerprinting setting</a> is disabled
        in brave://settings/shields </p>
    </div>
  </div>
  <div id="readergpt__gifContainer" class="align-middle mt-3 mb-3 d-none"> <span
      class="spinner-border text-primary mt-2" role="status"></span> <span
      class="align-middle readergpt__readableText">Summarising with ChatGPT</span> </div>
  <div id="readergpt__summaryTextHeader" class="m-3 d-none lead"> Used ChatGPT (<a href="${l}"
      target="_blank">Settings</a>) </div>
  <p id="readergpt__readableText" role="alert" class="readergpt__readableText alert alert-secondary d-none"></p> <a
    id="readergpt__buyMeACoffee"
    href="https://chrome.google.com/webstore/detail/summary-with-chatgpt-open/mikcekmbahpbehdpakenaknkkedeonhf/reviews"
    target="_blank" class="d-none d-flex justify-content-end m-3 link-secondary text-decoration-none">Rate us!</p>
</div>`;
      rootContainer.shadowRoot?.appendChild(containerEl);
      document.getElementsByTagName('body')[0]?.append(rootContainer);
      setTimeout((() => {
          var closeButtonEl;
          null === (closeButtonEl = getElement("readergpt__closeButton")) || void 0 === closeButtonEl || closeButtonEl.addEventListener("click", (async () => {
              ! function() {
                  var e;
                  let i = document.getElementById(shadowRootId);
                  null === (e = null == i ? void 0 : i.parentNode) || void 0 === e || e.removeChild(i)
              }()
          }));
          let navigator = window.navigator;
          navigator && navigator.brave && navigator.brave.isBrave && navigator.brave.isBrave()
              .then((e => {
                  var t;
                  e && (null === (t = getElement("readergpt__braveWarning")) || void 0 === t || t.classList.remove("d-none"))
              }))
      }), 500)
  })();
  if (parsedContent && parsedContent.textContent) {
    function a(e) {
      let port = chrome.runtime.connect();
      port.onMessage.addListener((msg => {
        var t, i;
        if ("DONE" === msg.event) return hideLoading();
        getElement('readergpt__summaryTextHeader')?.classList.remove("d-none");
        getElement('readergpt__buyMeACoffee')?.classList.remove("d-none");
        if (msg.text) {
          (function(e) {
              getElement("readergpt__readableText")?.classList.remove('d-none');
              getElement("readergpt__readableText")?.classList.add('alert-secondary');
              getElement("readergpt__readableText")?.classList.remove('alert-warning');
              getElement("readergpt__readableText")?.innerText = e;
          })(msg.text);
        } else if (msg.error) {
            hideLoading();
            try {
                let error = msg.error,
                    code = null == error ? void 0 : error.code,
                    message = null == error ? void 0 : error.message;
                if ("CLOUDFLARE" === code || "UNAUTHORIZED" === code) {
                  return void function() {
                      getElement('readergpt__loginContainer')?.classList.remove("d-none");
                  }();
                }
                if ("message_length_exceeds_limit" === code) {
                  return void showError("Article is too long and is not supported by ChatGPT yet. Stay tuned for a workaround.");
                }
                if (message) {
                  return void showError(message)
                }
            } catch (e) {}
      ...
      (function hideLogin() {
          var e, t, i;
          null === (e = getElement("readergpt__loginContainer")) || void 0 === e || e.classList.add("d-none"), null === (t = getElement("readergpt__summaryTextHeader")) || void 0 === t || t.classList.add("d-none"), null === (i = getElement("readergpt__buyMeACoffee")) || void 0 === i || i.classList.add("d-none")
      })();
      (function showHeader() {
          var e, t, i, a;
          null === (e = getElement("readergpt__gifContainer")) || void 0 === e || e.classList.remove("d-none"), null === (t = getElement("readergpt__gifContainer")) || void 0 === t || t.classList.add("loading"), null === (i = getElement("readergpt__loginContainer")) || void 0 === i || i.classList.add("loading"), null === (a = getElement("readergpt__readableText")) || void 0 === a || a.classList.add("d-none")
      })();
      port.postMessage({
          pageContent: e
      });
  ...
  showError("No readable text found on the page.")
 }
})()